# AGENTS.md - AI Agent Development Guide for tap-erpnext
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from ErpNext.

### Key Components

1. **Tap Class** (`tap_erpnext/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_erpnext/client.py`): Handles API communication and authentication
1. **Streams** (`tap_erpnext/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_erpnext/streams.py`
...
```python
class MyNewStream(ErpNextStream):
    name = "my_new_stream"
...
```bash
tap-erpnext --config config.json --discover
tap-erpnext --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_erpnext/tap.py`
...
```bash
# .env.example
TAP_ERPNEXT_API_URL=https://api.example.com
TAP_ERPNEXT_API_KEY=your_api_key_here
TAP_ERPNEXT_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-erpnext --version`), Python version, and a minimal reproduction case when reporting bugs.
